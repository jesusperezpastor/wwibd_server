...
let A;
const Q = 1 === new Uint8Array(new Uint16Array([1]).buffer)[0];
export function parse(g, I = "@") {
  ...
  const k = A.sa(D);
  if ((Q ? C : E)(g, new Uint16Array(A.memory.buffer, k, D)), !A.parseCJS(k, g.length, 0, 0, 0)) throw Object.assign(new Error(`Parse error ${I}${A.e()}:${g.slice(0, A.e()).split("\n").length}:${A.e() - g.lastIndexOf("\n", A.e() - 1)}`), {
    idx: A.e()
  });
  let w = new Set(),
    o = new Set(),
    J = new Set();
  for (; A.rre();) {
    const Q = B(g.slice(A.res(), A.ree()));
    Q && o.add(Q);
  }
  for (; A.ru();) J.add(B(g.slice(A.us(), A.ue())));
  for (; A.re();) {
    let Q = B(g.slice(A.es(), A.ee()));
    void 0 === Q || J.has(Q) || w.add(Q);
  }
  return {
    exports: [...w],
    reexports: [...o]
