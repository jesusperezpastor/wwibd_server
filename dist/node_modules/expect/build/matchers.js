...
const matchers = {
  ...
  toContain(received, expected) {
    ...
    if (typeof received === 'string') {
      ...
      if (typeof expected !== 'string') {
        throw new Error((0, _jestMatcherUtils.matcherErrorMessage)((0, _jestMatcherUtils.matcherHint)(matcherName, received, String(expected), options), wrongTypeErrorMessage,
        // eslint-disable-next-line prefer-template
  ...
  toContainEqual(received, expected) {
    ...
    const index = Array.from(received).findIndex(item => (0, _expectUtils.equals)(item, expected, [...this.customTesters, _expectUtils.iterableEquality]));
    const pass = index !== -1;
    const message = () => {
      const labelExpected = 'Expected value';
      const labelReceived = `Received ${(0, _jestGetType.getType)(received)}`;
